...
class UserAccessRightsController extends Controller
{
    ...
    {
        if (!empty(Session::get('admin'))) {
            $data['roles'] = Role_authorization::select('role_authorizations.*', 'modules.module_name', 'sub_modules.sub_module_name', 'module_configs.menu_name')
                ->leftJoin('modules', 'role_authorizations.module_name', '=', 'modules.id')
                ->leftJoin('sub_modules', 'role_authorizations.sub_module_name', '=', 'sub_modules.id')
                ->leftJoin('module_configs', 'role_authorizations.menu', '=', 'module_configs.id')
                ->distinct() // Use DISTINCT to avoid potential issues with GROUP BY
                ->get();

            return View::make('role.view-users-role', $data); // Adjust the view path based on your actual structure
        } else {
            return redirect('/');
        }
    }
